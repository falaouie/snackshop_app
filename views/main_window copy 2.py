...
class MainWindow(QMainWindow):
    ...
    def _setup_window_size(self):
        ...
        self.screen_available_width = screen_available.width()
        self.screen_height = screen_available.height()
        print(f"Width: {self.screen_available_width}")
        print(f"Height: {self.screen_height}")
        # Set minimum window size
        self.setMinimumSize(800, 600)

        # If screen_available is larger than minimum, use 80% of screen_available size
        if self.screen_available_width > 1024 and self.screen_height < 768:
            target_width = int(self.screen_available_width * 0.8)
    ...
    def _calculate_margin_size(self):
        ...
        if hasattr(self, 'screen_available_width') and self.screen_available_width > 1024:
            return int(base_margin * (self.screen_available_width / 1024) * 0.8)
        return base_margin
    ...
    def _calculate_logo_size(self):
        ...
        if hasattr(self, 'screen_available_width') and self.screen_available_width > 1024:
            scale_factor = min(self.screen_available_width / 1024, 2.0)  # Cap scaling at 2x
            return QSize(
